...
alter table album add column year_rel date;
...
INSERT INTO album (name, year_rel) 
VALUES('Конь', '2020-02-01');

INSERT into album (name, year_rel)
VALUES('Свое', '2022-02-01');

UPDATE album 
SET year_rel = '2013-02-01'
WHERE id = 8;
...
JOIN track t2 ON t.collection_id = t2.id 
JOIN album a ON t2.id = a.id
JOIN performeralbum p ON a.id = p.album_id 
JOIN performer p2 ON p.album_id = p2.id 
WHERE p2."name" LIKE '%Гагарина%'; 
